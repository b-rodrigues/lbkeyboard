...
# Load the package - supports both installed and development mode
if (requireNamespace("lbkeyboard", quietly = TRUE)) {
  library(lbkeyboard)
} else {
  devtools::load_all()
}
...
# Load QWERTZ-LUX from INI file
qwertz_lux <- parse_layout_ini(
  system.file("layouts/qwertz-lux/layout.ini", package = "lbkeyboard")
)

# If package install path doesn't work, try relative path
if (is.null(qwertz_lux$base) || nrow(qwertz_lux$base) == 0) {
...
effort_qwertz_lux <- calculate_layout_effort(qwertz_lux$base, all_texts,
                                             keys_to_evaluate = c(letters, "é", "à", "ç", "ü", "ö", "ä"),
                                             effort_weights = effort_weights)
...
if (!is.null(qwertz_lux$base) && nrow(qwertz_lux$base) > 0) {
  ...
  # Get keys by row
  layout_df <- qwertz_lux$base
  for (r in sort(unique(layout_df$row))) {
    row_keys <- layout_df[layout_df$row == r, ]
    row_keys <- row_keys[order(row_keys$number), ]
    cat(paste(toupper(row_keys$key), collapse = " "), "\n")
  }
  cat("```\n\n")
  
  # Also show AltGr if available
  if (!is.null(qwertz_lux$altgr) && nrow(qwertz_lux$altgr) > 0) {
    cat("**AltGr Layer (accented characters and symbols):**\n\n")
    cat("```\n")
    
    layout_df <- qwertz_lux$altgr
    for (r in sort(unique(layout_df$row))) {
      row_keys <- layout_df[layout_df$row == r, ]
      row_keys <- row_keys[order(row_keys$number), ]
      # Show only non-empty keys
      keys_display <- sapply(row_keys$key, function(k) {
        if (is.na(k) || k == "" || grepl("^dk", k) || k == "--") return("·")
        return(k)
      })
      cat(paste(keys_display, collapse = " "), "\n")
    }
    cat("```\n")
  }
}
