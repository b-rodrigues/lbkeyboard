...
— exactly what you'd want on your home row.

## How Much Better Is It?

Using the **lbkeyboard** package's effort model (which accounts for finger travel,
same-finger penalties, and ergonomic factors), here's how the layouts compare:
...
# QWERTZ-LUX heatmap - need to use keyboard template
qwertz_lux_kb <- ch_qwertz
letter_mask <- tolower(qwertz_lux_kb$key) %in% c(letters, "é", "ä", "ë", "ç")
letter_indices <- which(letter_mask)

# Map QWERTZ-LUX keys to QWERTZ positions for visualization
if (length(letter_indices) >= nrow(qwertz_lux)) {
  qwertz_lux_kb$key[letter_indices[1:nrow(qwertz_lux)]] <- qwertz_lux$key
  qwertz_lux_kb$key_label[letter_indices[1:nrow(qwertz_lux)]] <- toupper(qwertz_lux$key)
}

heatmap_qwertz_lux <- heatmapize(qwertz_lux_kb, freq_all)
